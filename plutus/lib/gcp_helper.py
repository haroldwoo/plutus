...
class GcpHelper:
    ...
    def get_project_number(self, project_id):
        ...
        try:
            ...
            metrics.incr("gcp_api_request_count",
                         tags=["type:resource_manager.fetch", f"project_id:{project_id}"])
            project_number = project_object.number
            return project_number
        except Exception as err:
            self.logger.error(f"Error fetching project {project_id}: {err}. Double check config.")
    ...
    def get_budgets_by_project(self, project, project_number):
        ...
        budgets = []
        try:
            parent = self.billing_client.billing_account_path(project.billing_account_id)
            for budget in self.billing_client.list_budgets(parent):
                if (f"projects/{project.project_id}" in budget.budget_filter.projects or
                        f"projects/{project_number}" in budget.budget_filter.projects):
                    budgets.append(budget)

            metrics.incr("gcp_api_request_count",
                         tags=["type:billing.list", f"project_id:{project.project_id}"])
        except Exception as err:
            self.logger.error(f"Error listing budgets for {project.billing_account_id}: {err}.")
            metrics.incr("error_count",
    ...
    def sync_config_with_gcp_budget(self, project, budget_dict, project_number):
        ...
        if (f"projects/{project.project_id}" not in budget_dict['budget_filter']['projects'] and
                f"projects/{project_number}" not in budget_dict['budget_filter']['projects']):
            self.logger.error("Project id/num not found, shouldn't happen...")
            metrics.incr("error_count", tags=["type:misconfig", f"project_id:{project.project_id}"])
            return False, None
        ...
        if project.budget_type == 'AMT':
            ...
            elif budget_dict['amount']['specified_amount']['units'] != project.budget_amount:
                self.logger.info("Change detected in budget units amount")
                ret_val = False
                changed_budget['amount']['specified_amount']['units'] = project.budget_amount
        ...
        if project.include_credits:
            if budget_dict['budget_filter']['credit_types_treatment'] != 'INCLUDE_ALL_CREDITS':
                self.logger.info("Change in include_credits. Config=True, GCP=False, updating...")
                ret_val = False
                changed_budget['budget_filter']['credit_types_treatment'] = 'INCLUDE_ALL_CREDITS'
        elif project.include_credits is False:
            if budget_dict['budget_filter']['credit_types_treatment'] != 'EXCLUDE_ALL_CREDITS':
                self.logger.info("Change in include_credits. Config=False, GCP=True, updating...")
                ret_val = False
                changed_budget['budget_filter']['credit_types_treatment'] = 'EXCLUDE_ALL_CREDITS'
        ...
        if any(x != y for x, y in pairs) or len(config_thresholds) != len(budget_thresholds):
        ...
        if project.pubsub:
            if budget_dict.get('all_updates_rule'):
                # GCP has changed the API object recently so that this key doesn't always exist.
                if (budget_dict['all_updates_rule'].get('pubsub_topic') is None or
                        budget_dict['all_updates_rule'].get('schema_version') is None):
                    self.logger.info("Pubsub changed to True in config but not setup in GCP.")
                    ret_val = False

                    changed_budget['all_updates_rule'] = {'pubsub_topic': pubsub_topic,
                                                          'schema_version': '1.0'}
                elif (budget_dict['all_updates_rule'].get('pubsub_topic') == pubsub_topic and
                      budget_dict['all_updates_rule'].get('schema_version') == '1.0'):
                ...
                # Configured pubsub to true but the all_updates_rule key in GCP doesn't exist.
                ret_val = False
                changed_budget['all_updates_rule'] = {'pubsub_topic': pubsub_topic,
                                                      'schema_version': '1.0'}
            ...
            if budget_dict.get('all_updates_rule'):
                # GCP has changed the API object recently so that this key doesn't always exist.
                if (budget_dict['all_updates_rule'].get('pubsub_topic') is not None or
                        budget_dict['all_updates_rule'].get('schema_version') is not None):
                    self.logger.info("Pubsub chanted to False in config, but not in GCP.")
                    self.logger.info("Note: Budget API bug exists where pubsub will not be \
                                      removed, even though the API call succeeds.")
                    ret_val = False
                    changed_budget['all_updates_rule'] = {}
    ...
    def create_budget(self, project):
        ...
        for rule in project.threshold_rules:
            threshold_rules.append({'threshold_percent': rule['threshold_percent'],
        ...
        if project.pubsub:
            ...
            all_updates_rule = {'pubsub_topic': project.pubsub_topic,
                                'schema_version': '1.0'}
        else:
            all_updates_rule = {}

        budget = {'display_name': project.display_name,
                  'budget_filter': {'projects': [f"projects/{project.project_id}"],
        ...
        try:
            response = self.billing_client.create_budget(parent, budget)
            metrics.incr("gcp_api_request_count",
                         tags=["type:billing.create", f"project_id:{project.project_id}"])
            return response
        except GoogleAPICallError as err:
            self.logger.error(f"Create budget request failed. \
                              GoogleAPICallError: {err}. Project id: {project.project_id}")
            metrics.incr("gcp_api_error_count",
                         tags=["type:billing.create.apicall", f"project_id:{project.project_id}"])
        except RetryError as err:
            self.logger.error(f"Request failed due to retryable error \
                              and retry attempts failed: {err}")
            metrics.incr("gcp_api_error_count",
                         tags=["type:billing.create.retry", f"project_id:{project.project_id}"])
        except ValueError as err:
            self.logger.error(f"Requst failed likely due to invalid parameters. ValueError: {err}")
            metrics.incr("gcp_api_error_count",
    ...
    def update_budget(self, changed_budget):
        ...
        try:
            response = self.billing_client.update_budget(changed_budget)
            metrics.incr("gcp_api_request_count",
                         tags=["type:billing.update",
                               f"display_name:{changed_budget['display_name']}"])
            return response
        except GoogleAPICallError as err:
            self.logger.error(f"Update budget request failed due to GoogleAPICallError: {err}. \
                              Budget was {changed_budget['display_name']}")
            metrics.incr("gcp_api_error_count",
                         tags=["type:billing.update.apicall",
                               f"display_name:{changed_budget['display_name']}"])
        except RetryError as err:
            self.logger.error(f"Request failed due to retryable error \
                              and retry attempts failed: {err}")
            metrics.incr("gcp_api_error_count",
                         tags=["type:billing.update.retry",
                               f"display_name:{changed_budget['display_name']}"])
        except ValueError as err:
            self.logger.error(f"Requst failed likely due to invalid parameters. ValueError: {err}")
            metrics.incr("gcp_api_error_count",
        ...
        if len(budgets) == 0:
            ...
            for budget in budgets:
                if budget.display_name == project.display_name:
                    ...
                    budget_dict = protobuf_to_dict(budget, use_enum_labels=True)
                    ...
                    if not same and changed_budget is not None:
                        self.logger.info(f"Changes detected. Updating budget to {changed_budget}")
                        new_budget = self.update_budget(changed_budget)
                        return new_budget
                    elif changed_budget is None:
                        self.logger.error("Error while comparing config and gcp budget. Exiting...")
                        sys.exit(1)
                    else:
                        self.logger.debug(f"No changes with budget {budget_dict['display_name']}")
                        return budget

            # No budgets for project matched the config, create new budget
            self.logger.info(f"No plutus budget found for {project.display_name}, creating...")
