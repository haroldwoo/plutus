...
def upsert_budget(mysql_cursor, budget, project_id, config_type, alert_emails):
    ...
    if ('projects' in budget_dict['budget_filter'] and
            len(budget_dict['budget_filter']['projects']) == 1):
        project_number = budget_dict['budget_filter']['projects'][0].replace("projects/", "")
    else:
        # This is a design decision based on the budgets API which only allows 0(ALL) or 1 project
        log.error("Unexpected error. Plutus should only manage budgets for single projects.")
        metrics.incr("error_count", tags=["type:project_budget_filter", f"project_id:{project_id}",
    ...
    elif budget_dict['budget_filter']['credit_types_treatment'] == 'EXCLUDE_ALL_CREDITS':
        include_credits = "FALSE"
    else:
        log.error("Unexpected error. Credits_type_treatment should be either of \
                  INCLUDE_ALL_CREDITS or EXCLUDE_ALL_CREDITS.")
        log.error("Google may have changed their budgets API.")
        include_credits = "FALSE"
        metrics.incr("error_count", tags=["type:credits_type_treatment",
                                          f"project_id:{project_id}", f"config_type:{config_type}"])

    if budget_dict['all_updates_rule'].get('pubsub_topic') is None:
        pubsub = "FALSE"
        pubsub_topic = "NA"
    else:
        pubsub = "TRUE"
        pubsub_topic = budget_dict['all_updates_rule']['pubsub_topic']
...
"""
    ...
    try:
        ...
        metrics.incr("upsert_count", tags=[f"project_id:{project_id}",
                                           f"config_type:{config_type}"])
    except OperationalError as err:
        log.fatal(f"Operational error while running query: {sql}. Error: {err}")
        metrics.incr("error_count", tags=["type:sql_operational_err", f"project_id:{project_id}",
                                          f"config_type:{config_type}"])
    except DatabaseError as err:
        log.fatal(f"Database error while running query: {sql}. Error: {err}")
        metrics.incr("error_count", tags=["type:sql_db_err", f"project_id:{project_id}",
                                          f"config_type:{config_type}"])
    except Error as err:
        log.fatal(f"Exception while running query: {sql}. Error: {err}")
        metrics.incr("error_count", tags=["type:sql_exception", f"project_id:{project_id}",
                                          f"config_type:{config_type}"])


def get_owner_emails_for_project(project_id, alert_emails, default="dataops@mozilla.com"):
    ...
    metrics.incr("gcp_api_request_count", tags=["type:resource_manager.getIamPolicy",
