...
def verify_project_yaml(project_dict):
    if "project_id" not in project_dict:
        ...
        log.error(
            f"Config error. 'project_id' value must be str. \
...
def verify_parent_yaml(parent_dict):
    ...
    for k in keys:
        if k in parent_dict:
            log.error(
                "Config error. Parent config should not include project_id or label_list."
            )
            return False
...
def verify_labels_yaml(label_dict):
    ...
    for k in keys:
        if k in label_dict:
            log.error(
                "Config error. Label config should not \
    ...
    if "label_list" not in label_dict:
        ...
        log.error(
            f"Config error. 'label_list' value must be of type list. \
        ...
        for i in label_dict["label_list"]:
            if not isinstance(i, dict):
                log.error(
                    f"Config error. Labels in label_list must be of type dict. \
                          E.g. 'key':'value'. Got {i}."
                )
                return False
            elif len(i) != 1:
                log.error(
                    "Config error. Labels in label_list \
...
def verify_default_yaml(default_dict):
    ...
    for k in keys:
        if k in default_dict:
            log.error(
                "Config error. Default config should not include \
...
def _verify_budget_values(project_dict):
    ...
    if not isinstance(project_dict["budget_type"], str):
        log.error(
            f"Config error. budget_type {project_dict['budget_type']} must be type str."
        )
        return False

    if not isinstance(project_dict["budget_amount"], int):
        log.error(
            f"Config error. budget_amount {project_dict['budget_amount']} must be type int."
        )
        return False
    ...
    if (project_dict["budget_type"] != "AMT") and (
        project_dict["budget_type"] != "LASTMONTH"
    ):
        log.error(
            f"Config error. budget_type {project_dict['budget_type']} \
    ...
    if "products" in project_dict:
        if not isinstance(project_dict["products"], list):
            log.error(
                f"Config error. 'products' type {type(project_dict['products'])} \
    ...
    if "alert_emails" in project_dict:
        if not isinstance(project_dict["alert_emails"], list):
            log.error(
                f"Config error. 'alert_emails' type {type(project_dict['alert_emails'])} \
                      must be of type list(str)."
            )
            return False
    ...
    if "threshold_rules" in project_dict:
        if not isinstance(project_dict["threshold_rules"], list):
            log.error(
                f"Config error. 'threshold_rules' type {type(project_dict['threshold_rules'])} \
        ...
        for rule in project_dict["threshold_rules"]:
            if rule.get("spend_basis") and rule.get("threshold_percent"):
                if not (
                    ...
                    or rule["spend_basis"] == "FORECASTED_SPEND"
                ):
                    log.error(
                        f"Config error. 'spend_basis' must be either \
                              CURRENT_SPEND or FORECASTED_SPEND'. Got '{rule['spend_basis']}'."
                    )
                    return False
                if not isinstance(rule["threshold_percent"], float):
                    log.error(
                        f"Config error. 'threshold_percent' must be of type float. \
                ...
                log.error(
                    "Config error. 'threshold_rules' must contain a list \
    ...
    if "pubsub_topic" in project_dict:
        if not isinstance(project_dict["pubsub_topic"], str):
            log.error(
                f"Config error. 'pubsub_topic' must be of type str. \
