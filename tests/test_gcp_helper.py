...
def project():
    ...
    return ProjectBudget(project_dict, PLUTUS_CONFIG_TYPE_PROJECT,
                         'foo-bar-123', 'project/some-project/topics/foobar')


def gen_budget_dict(project_id_or_num='some-project-id',
    ...
    dict = {'name': 'billingAccounts/foo-bar-123/budgets/some-budget-id-hash',
            'display_name': 'plutus-doesnt-matter',
            'budget_filter': {'projects': ['projects/' + project_id_or_num],
                              'credit_types_treatment': credit_types_treatment},
            'amount': amount,
            'threshold_rules': [{'threshold_percent': 1.1,
                                 'spend_basis': 'CURRENT_SPEND'},
                                {'threshold_percent': 1.2,
                                 'spend_basis': 'FORECASTED_SPEND'}],
            'all_updates_rule': {'pubsub_topic': 'projects/some-project-id/topics/some-topic',
                                 'schema_version': '1.0'},
            'etag': 'xxxyyyzzz'
            }
    return dict
...
def test_match(project):
    ...
    same, returned_budget = gcp_helper.sync_config_with_gcp_budget(project, budget_dict, '12345')
    assert(same is True)
...
def test_toggle_include_credits_true(project):
    ...
    assert(budget_dict['budget_filter']['credit_types_treatment'] == 'EXCLUDE_ALL_CREDITS')
    same, returned_budget = gcp_helper.sync_config_with_gcp_budget(project, budget_dict, '12345')
    assert(same is False)
    assert(returned_budget['budget_filter']['credit_types_treatment'] == 'INCLUDE_ALL_CREDITS')
...
def test_toggle_include_credits_false(project):
    ...
    same, returned_budget = gcp_helper.sync_config_with_gcp_budget(project, budget_dict, '12345')
    assert(same is False)
    assert(returned_budget['budget_filter']['credit_types_treatment'] == 'EXCLUDE_ALL_CREDITS')


def test_update_threshold_rules(project):
    budget_dict = gen_budget_dict()
    project.threshold_rules.append({'threshold_percent': 1.3, 'spend_basis': 'CURRENT_SPEND'})
    ...
    budget_list = sorted(
        returned_budget['threshold_rules'], key=lambda x: x['threshold_percent'], reverse=True
    )
...
def test_toggle_pubsub_true(project):
    ...
    budget_dict['all_updates_rule'] = {}
    assert(project.pubsub is True)
    assert(budget_dict['all_updates_rule'] == {})
    same, returned_budget = gcp_helper.sync_config_with_gcp_budget(project, budget_dict, '12345')
    assert(same is False)
    assert(returned_budget['all_updates_rule'] ==
           {'pubsub_topic': 'projects/some-project-id/topics/some-topic', 'schema_version': '1.0'})
